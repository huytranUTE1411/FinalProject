...
SerialCommand::SerialCommand(void * uart_device)
{
  hwSerial = (HardwareSerial*)uart_device;
...
void SerialCommand::readSerial() 
{
	...
	#else
	while ((usingSoftwareSerial==0 && hwSerial->available() > 0) || (usingSoftwareSerial==1 && SoftSerial->available() > 0) )
	#endif
	{
		int i; 
		boolean matched; 
		if (usingSoftwareSerial==0) {
			...
			inChar=hwSerial->read();   // Read single available character, there may be more waiting
		} else {
			#ifndef SERIALCOMMAND_HARDWAREONLY
			// SoftwareSerial port
		...
		hwSerial->print(inChar);   // Echo back to serial stream
		#endif
		if (inChar==term) {     // Check for the terminator (default '\r') meaning end of command
			...
			for (i=0; i<numCommand; i++) {
				#ifdef SERIALCOMMANDDEBUG
				hwSerial->print("Comparing ["); 
				hwSerial->print(token); 
				hwSerial->print("] to [");
				hwSerial->print(CommandList[i].command);
				hwSerial->println("]");
				#endif
				...
				{
					#ifdef SERIALCOMMANDDEBUG
					hwSerial->print("Matched Command: "); 
					hwSerial->println(token);
					#endif
...
void SerialCommand::addCommand(const char *command, void (*function)())
{
	if (numCommand < MAXSERIALCOMMANDS) {
		#ifdef SERIALCOMMANDDEBUG
		hwSerial->print(numCommand); 
		hwSerial->print("-"); 
		hwSerial->print("Adding command for "); 
		...
		// the command
		#ifdef SERIALCOMMANDDEBUG
		hwSerial->println("Too many handlers - recompile changing MAXSERIALCOMMANDS"); 
		#endif 
	}
}
